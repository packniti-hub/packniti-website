...
function slugFor(p){const d=p?.dimensions||{};if(d.length==null||d.breadth==null||d.width==null)return"";const n=v=>{const x=Number(v);return Number.isInteger(x)?String(x):String(x).replace(/\.0+$/,'')};return `${n(d.length)}x${n(d.breadth)}x${n(d.width)}`}
function imageUrl(raw,p){const slug=slugFor(p);if(slug)return `assets/product-gallery/${slug}.png`;const s=String(raw||"").trim();if(!s)return"";if(s.startsWith("assets/")||s.startsWith("./assets/")||s.startsWith("../assets/"))return s;const m=s.match(/drive\.google\.com\/.*[?&]id=([^&]+)/i)||s.match(/drive\.google\.com\/file\/d\/([^/]+)/i);return m?`https://drive.google.com/thumbnail?id=${encodeURIComponent(m[1])}&sz=w400`:s}
function price(p,qty){const tiers=p.pricing||[];if(qty<=100)return Number(tiers.find(t=>t.range==="1-100")?.price_inr||0);if(qty<=500)return Number(tiers.find(t=>t.range==="101-500")?.price_inr||0);if(qty<=2000)return Number(tiers.find(t=>t.range==="501-2000")?.price_inr||0);return Number((tiers.find(t=>t.range==="2000+")?.price_inr ?? tiers.find(t=>t.range==="2001+")?.price_inr ?? 0))}
...
function changeQty(i,delta){items[i].qty=Math.max(1,(Number(items[i].qty)||1)+delta*100);save();render()}
