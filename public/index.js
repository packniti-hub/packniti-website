...
const localImage=p=>{
  ...
  const known={'5x3x2':'assets/catalogue/box_5x3x2.jpg','4x3x3':'assets/catalogue/box_4x3x3.jpg','6.5x2.5x3':'assets/catalogue/box_6.5x2.5x3.png','5x4x3.5':'assets/catalogue/box_5x4x3.5.jpg','6x4x3':'assets/catalogue/box_6x4x3.jpg','8x4x1.5':'assets/catalogue/box_8x4x1.5.jpg'};
  return known[key]||String(p?.image_url||'');
};
const slugOf=p=>[p?.dimensions?.length,p?.dimensions?.breadth,p?.dimensions?.width].map(v=>{const n=Number(v);return Number.isInteger(n)?String(n):String(v).replace(/\.0+$/,'')}).join('x');
...
fetch('data/catalogue.json').then(r=>r.json()).then(data=>{
  ...
  const keyOf=p=>slugOf(p);
  const list=wanted.map(k=>products.find(p=>keyOf(p)===k)).filter(Boolean);
  grid.innerHTML=list.map(p=>`<a class="feature" href="boxes/${encodeURIComponent(slugOf(p))}.html">
    ...
    <div class="feature-foot"><div class="feature-price"><strong>${price(p)?money(price(p)):'Enquiry'}</strong><span> / box</span></div><span class="feature-link">View details →</span></div>
  </a>`).join('');
